...
import { getMetabolicAdvice, metabolicLoop } from './modules/engine.js'
import {
  render,
  renderFineLog,
  updateStepDisplay,
...
function calculateMacrosCalories(macros = {}) {
  ...
  return {
    ...
    protein: proteins,
    fat: fats,
    calories: Math.round(totalCarbs * 4 + proteins * 4 + fats * 9)
  }
}
...
function updateCaloriesPreview(grams, calPer100) {
  ...
  const gramsValue = Number.isFinite(Number(grams)) ? Number(grams) : 0
  const baseCal = Number.isFinite(Number(calPer100)) ? Number(calPer100) : 0
  const total = Math.round((baseCal * gramsValue) / 100)
  caloriesPreviewNode.textContent = `Примерно ${total} kcal`
}
...
function fillFoodFields(values = {}, options = {}) {
  ...
  if (!preserveGrams && values.grams !== undefined) {
    ...
    if (gramsInput) {
      const numeric = Number(values.grams)
      gramsInput.value = Number.isFinite(numeric) ? numeric.toString() : ''
    }
  }
  if (!preserveCalories && values.calPer100 !== undefined) {
    const calInput = document.getElementById('calPer100')
    if (calInput) {
      const numeric = Number(values.calPer100)
      calInput.value = Number.isFinite(numeric) ? numeric.toString() : ''
    }
  }
  MACRO_FIELDS.forEach((field) => {
    ...
    input.value = Number.isFinite(numeric) ? formatMacroInput(numeric) : ''
  })
...
function refreshMetabolicStatus(modeOverride, providedLoopData) {
